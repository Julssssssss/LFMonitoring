  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex justify-center items-center" onClick={onClose}>
      ...
      <div className="max-w-full max-h-full" onClick={(e) => e.stopPropagation()}>
        <img src={url[currentSlide]} alt={`Image ${currentSlide + 1}`} className="w-[17rem] h-[17rem] sm:w-[22rem] sm:h-[22rem] md:w-[40rem] md:h-[30rem] object-contain xl:w-[55rem] xl:h-[45rem]" />
      </div>
              ...
              {images.map((url, index) => (
                <div key={index}>
                  ...
                  {enableDeleteButton && (
                    <button
                      type="button"
                      className="text-white text-[1rem] absolute top-2 right-[2rem] rounded-sm bg-black hover:bg-[#F9D62B] hover:text-black text-center h-[1.5rem] w-[1.5rem] xsm:top-4 sm:top-1"
