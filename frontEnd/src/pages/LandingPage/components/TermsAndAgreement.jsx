...
const TermsAndAgreement = ({closeTAC}) => {
  ...
  return (
    <div className="fixed z-40 w-screen h-screen flex flex-col justify-center items-center">
      <div className='h-auto w-[20rem] absolute z-50 bg-[#0D1E48] border-2 border-[#F9D62B] rounded-xl text-white xl:w-[30rem] xl:h-[40rem] 2xl:w-[45rem] 2xl:h-[50rem]'>
        <div className='flex flex-col items-center'>
          <div className="w-full flex flex-row justify-end">
            <button onClick={closeTAC} className='h-[1.5rem] w-[1.5rem] m-[0.5rem] rounded-full xl:h-[2rem] 2xl:h-[3rem] xl:w-[3rem]'>
          ...
          <button onClick={handleAcceptTAC} className='bg-[#003985] hover:bg-[#F9D62B] hover:text-black h-[2rem] text-white w-[7rem] rounded-md font-poppins mb-[1rem]'>Accept</button>
        </div>
      </div>
    </div>
  )
