...
router.post('/refreshToken', async(req, res)=>{
    ...
    jwt.verify (refreshToken.jwt, process.env.JWT_REFRESH_SECRET, (err, user)=>{
        ...
        const accessToken = jwt.sign(data, process.env.JWT_ACCESS_SECRET, {expiresIn: '600s'}) // 600
        console.log('success')
...
router.post("/login/success", async(req, res)=>{
    ...
    if(req.session.userId){
        ...
        .then(async(result)=>{
            ...
            const accessToken = jwt.sign(userData, process.env.JWT_ACCESS_SECRET, {expiresIn: '900s'}) // 900
            const refreshToken = jwt.sign(userData, process.env.JWT_REFRESH_SECRET, {expiresIn: '960s'}) //15 minutes  960
